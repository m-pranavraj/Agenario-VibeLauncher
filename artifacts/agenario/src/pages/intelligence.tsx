...
function DonutChart({ segments }: { segments: Array<{ label: string; count: number; color: string }> }) {
  ...
  if (total === 0) return <div className="w-24 h-24 rounded-full bg-white/[0.04] border border-white/[0.07]" />;
  ...
  return (
    <div className="relative flex items-center justify-center" style={{ width: 96, height: 96 }}>
      ...
      <div className="absolute text-center">
        <div className="text-lg font-bold text-white font-['Syne']">{total}</div>
    ...
    <div className="flex items-end gap-1.5 h-16">
      {bars.map((b, i) => (
        <div key={i} className="flex flex-col items-center gap-1 flex-1">
          ...
          <span className="text-[8px] text-white/25 text-center truncate w-full">{b.label}</span>
        </div>
      ))}
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-[#050505]">
      ...
      <nav className="border-b border-white/[0.07] bg-[#050505]/90 backdrop-blur-2xl sticky top-0 z-10">
        <div className="max-w-5xl mx-auto px-6 py-4 flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-white font-bold font-['Syne'] text-sm">Intelligence Hub</span>
          </div>
          <div className="ml-auto flex items-center gap-3">
            <Link href="/scans/new">
              <button className="flex items-center gap-1.5 bg-white text-black text-xs font-semibold px-4 py-2 rounded-xl hover:bg-white/90 transition-colors">
      ...
      <main className="max-w-5xl mx-auto px-6 py-10 space-y-6">
        <div>
          <h1 className="text-2xl font-bold text-white font-['Syne']">Intelligence Hub</h1>
        ...
        {scansLoading ? (
          ...
          <div className="text-center py-24 glass rounded-2xl">
            ...
            <h3 className="text-white font-bold font-['Syne'] mb-2">No scan data yet</h3>
            <p className="text-white/30 text-sm mb-6">Run your first scan to populate your intelligence dashboard.</p>
            <Link href="/scans/new">
              <button className="bg-white text-black font-semibold px-6 py-2.5 rounded-xl hover:bg-white/90 transition-colors text-sm">
                Run First Scan
              </button>
            </Link>
          </div>
          ...
          <>
              ...
              {[
                {
                  ...
                  color: avgScore >= 80 ? "text-green-400" : avgScore >= 55 ? "text-amber-400" : "text-red-400",
                  icon: Target,
                  trend: scoreDelta,
                },
                {
                ...
                },
                {
                  ...
                  color: criticalTotal > 0 ? "text-red-400" : "text-green-400",
                  icon: AlertTriangle,
                  trend: null,
                },
                {
                  ...
                  color: readyCount > 0 ? "text-green-400" : "text-white/40",
                  icon: CheckCircle2,
                  trend: null,
                },
              ].map((kpi) => (
                <motion.div key={kpi.label} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}
                  ...
                  <div className="flex items-end gap-2">
                    ...
                    {kpi.trend != null && (
                      <div className={`flex items-center gap-0.5 text-xs mb-0.5 ${kpi.trend >= 0 ? "text-green-400" : "text-red-400"}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}
                ...
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h3 className="text-sm font-bold text-white font-['Syne'] flex-1">Score Trend</h3>
                  <ScoreSparkline scores={[...scores].reverse()} />
                </div>
                {chartBars.length > 0 ? (
                  <BarChart bars={chartBars} />
                ) : (
                  <p className="text-white/25 text-xs text-center py-6">Not enough data</p>
                )}
              ...
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.07 }}
                ...
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h3 className="text-sm font-bold text-white font-['Syne']">Issue Distribution</h3>
                </div>
                <div className="flex items-center gap-6">
                  ...
                  <div className="space-y-2">
                    {[
                      { label: "Critical", count: criticalTotal, color: "bg-red-400", text: "text-red-400" },
                      { label: "High", count: highTotal, color: "bg-amber-400", text: "text-amber-400" },
                      { label: "Medium", count: mediumTotal, color: "bg-yellow-400", text: "text-yellow-400" },
            ...
            {isCreator && (hasPredictive || hasTwin) && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
                ...
                <div className="px-6 py-4 border-b border-white/[0.05] flex items-center gap-3">
                  ...
                  <h3 className="text-sm font-bold text-white font-['Syne']">Deep Tech Engine Aggregates</h3>
                  <span className="text-xs text-white/25 ml-auto">Across {Math.max(predictiveAvg.count, twinStats.count)} scans</span>
                </div>
                <div className="grid grid-cols-2 sm:grid-cols-5 divide-x divide-white/[0.05]">
                  {[
                    hasPredictive && { icon: Target, label: "Avg Release Confidence", value: `${avgConfidence}/100`, color: (avgConfidence ?? 0) >= 70 ? "text-green-400" : "text-amber-400" },
                    hasPredictive && { icon: AlertTriangle, label: "Avg Outage Probability", value: `${avgOutage}%`, color: (avgOutage ?? 0) >= 50 ? "text-red-400" : "text-amber-400" },
                    hasPredictive && { icon: TrendingDown, label: "Avg Churn Risk", value: `${avgChurn}%`, color: (avgChurn ?? 0) >= 25 ? "text-amber-400" : "text-green-400" },
                    hasTwin && { icon: RefreshCw, label: "Avg Journey Pass Rate", value: `${Math.round(twinStats.journeyPass / twinStats.count)}%`, color: "text-violet-400" },
                    hasTwin && { icon: Shield, label: "Avg Attack Block Rate", value: `${Math.round(twinStats.attackBlock / twinStats.count)}%`, color: "text-violet-400" },
                  ].filter(Boolean).map((stat: any, i) => (
                    <div key={i} className="px-5 py-4 text-center">
                      <stat.icon className="w-4 h-4 text-white/20 mx-auto mb-2" />
                      <div className={`text-xl font-bold font-['Syne'] ${stat.color}`}>{stat.value}</div>
                      <div className="text-[10px] text-white/25 mt-1 leading-tight">{stat.label}</div>
                    </div>
                  ))}
                </div>
              </motion.div>
            ...
            {!isCreator && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
                className="glass rounded-2xl p-5 flex items-center gap-4 border border-violet-500/15">
                <Brain className="w-8 h-8 text-violet-400/50 shrink-0" />
                <div className="flex-1">
                  <p className="text-white font-semibold text-sm">Unlock Deep Tech Aggregates</p>
                  <p className="text-white/35 text-xs mt-0.5">Predictive Intelligence, Digital Twin stats, and Root Cause chains require Creator plan.</p>
                </div>
                <Link href="/pricing">
                  <button className="flex items-center gap-1.5 bg-white text-black text-xs font-semibold px-4 py-2 rounded-lg hover:bg-white/90 transition-colors shrink-0">
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.12 }}
              ...
              <div className="px-6 py-4 border-b border-white/[0.05] flex items-center gap-2">
                ...
                <h3 className="text-sm font-bold text-white font-['Syne']">All Scans</h3>
              </div>
                    ...
                    <Link key={scan.id} href={`/scans/${scan.id}`}>
                      <div className="px-6 py-3 flex items-center gap-4 hover:bg-white/[0.02] transition-colors cursor-pointer">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-sm text-white/70 truncate">{scan.sourceInput}</p>
                          <div className="flex items-center gap-2 mt-0.5">
                            <span className="text-[10px] text-white/25 capitalize">{scan.sourceType}</span>
                            {scan.launchVerdict && (
                              <span className={`text-[10px] font-medium ${scan.launchVerdict === "ready" ? "text-green-400" : scan.launchVerdict === "caution" ? "text-amber-400" : "text-red-400"}`}>
                        ...
                        {pi && isCreator && (
                          <div className="hidden sm:flex items-center gap-3 text-[10px] text-white/30">
                            <span className={pi.releaseConfidenceScore >= 70 ? "text-green-400" : "text-amber-400"}>
                              {pi.releaseConfidenceScore}% conf
                            </span>
                            <span className={pi.outageProbability >= 50 ? "text-red-400" : "text-white/30"}>
                              {pi.outageProbability}% outage
                            </span>
                          </div>
                        )}
                        <div className="flex items-center gap-1.5 text-[10px] text-white/20">
                          {scan.issueCounts?.critical ? <span className="text-red-400">{scan.issueCounts.critical}C</span> : null}
                          {scan.issueCounts?.high ? <span className="text-amber-400">{scan.issueCounts.high}H</span> : null}
