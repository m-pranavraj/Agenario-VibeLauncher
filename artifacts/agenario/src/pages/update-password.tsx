...
export default function UpdatePasswordPage() {
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await api.auth.updatePassword({ token, newPassword: password });
      setSuccess(true);
      setTimeout(() => {
        setLocation("/login");
      }, 3000);
        ...
        <div className="w-full max-w-[420px]">
          <div className={`${t.card} relative overflow-hidden`}>
            ...
            <div className="text-center mb-8 relative z-10">
              <h1 className={`text-2xl font-bold font-['Syne'] mb-2 ${t.heading}`}>Create New Password</h1>
            ...
            <AnimatePresence mode="wait">
              {success && (
                ...
                >
                  ...
                  <p className="text-sm font-medium leading-relaxed">Password updated successfully! Redirecting to login...</p>
                </motion.div>
              )}
            </AnimatePresence>
            ...
            <form onSubmit={handleUpdate} className="space-y-5 relative z-10">
              <div>
                <label className={`block text-xs font-medium mb-2 uppercase tracking-wide ${t.label}`}>New Password</label>
              ...
              <div>
                <label className={`block text-xs font-medium mb-2 uppercase tracking-wide ${t.label}`}>Confirm Password</label>
                <div className="relative">
                  <Lock className={`absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 ${t.icon}`} />
                  <input
                    type="password"
