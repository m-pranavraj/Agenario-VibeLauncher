...
export default function DashboardPage() {
  ...
  const t = {
    ...
    issueLow:    "text-[11px] text-white/25",
  };
  ...
  return (
    <div className={t.page}>
      <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden">
        <div className={`absolute top-[-15%] left-[-5%] w-[50%] h-[50%] blur-[180px] rounded-full ${"bg-violet-600/[0.05]"}`} />
        <div className={`absolute bottom-[-10%] right-[-5%] w-[40%] h-[40%] blur-[160px] rounded-full ${"bg-blue-500/[0.04]"}`} />
      </div>
      ...
      <nav className={`sticky top-0 z-50 ${t.nav}`}>
        <div className="max-w-5xl mx-auto px-6 h-14 flex items-center gap-6">
          ...
          <div className="flex items-center gap-3 ml-auto">
            ...
            <span className={`text-xs hidden sm:block ${"text-white/25"}`}>{user.email}</span>
            {isFreePlan && (
      ...
      <main className="relative z-10 max-w-5xl mx-auto px-6 py-10">
        ...
        {scansLoading ? (
          ...
          >
            <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mx-auto mb-4 border ${"bg-white/[0.06] border-white/[0.1]"}`}>
