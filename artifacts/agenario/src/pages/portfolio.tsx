...
function ScoreBar({ score }: { score: number | null, false?: boolean }) {
  const pct = score ?? 0;
  ...
  return (
    <div className={`min-h-screen ${"bg-[#050505] text-white"}`}>
      ...
      <nav className={`border-b ${"bg-[#050505]/90 border-white/[0.07]"} backdrop-blur-2xl sticky top-0 z-10`}>
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className={`font-bold font-['Syne'] text-sm ${"text-white"}`}>Risk Portfolio</span>
          </div>
          <div className="ml-auto">
            <Link href="/scans/new">
              <button className={`flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-lg transition-all ${"bg-white text-black hover:bg-white/90"}`}>
      ...
      <main className="max-w-6xl mx-auto px-6 py-8 space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            ...
            <motion.div key={s.label} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
              className={`${"glass aurora-card"} rounded-2xl p-5`}>
              <div className={`text-3xl font-bold font-['Syne'] ${s.color}`}>{s.value}</div>
              <div className={`text-xs mt-1 ${"text-white/30"}`}>{s.label}</div>
            </motion.div>
          ))}
        </div>
        ...
        {/* Sort controls */}
        <div className="flex items-center gap-2">
          <span className={`text-xs uppercase tracking-widest font-medium mr-1 ${"text-white/25"}`}>Sort by</span>
        ...
        {fetching ? (
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
            ...
            <h3 className={`font-bold font-['Syne'] mb-2 ${"text-white"}`}>No apps tracked yet</h3>
            <p className={`text-sm mb-6 ${"text-white/30"}`}>Run your first scan to start building your risk portfolio.</p>
            <Link href="/scans/new">
              <button className={`font-semibold text-sm px-6 py-2.5 rounded-xl transition-all ${"bg-white text-black hover:bg-white/90"}`}>
              ...
              return (
                <motion.div key={app.scanId} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}>
                  <Link href={`/scans/${app.scanId}`}>
                    <div className={`${"glass scan-card-aurora hover:bg-white/[0.04]"} rounded-xl p-5 transition-all cursor-pointer`}>
                      <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1">
                            <SourceIcon className={`w-3.5 h-3.5 shrink-0 ${"text-white/25"}`} />
                            <span className={`text-sm font-medium truncate ${"text-white/80"}`}>{app.source}</span>
                            {vc && (
                          ...
                          <div className={`flex items-center gap-3 text-xs mb-3 ${"text-white/30"}`}>
        ...
        {portfolio.length > 0 && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}
            className={`${"glass aurora-card aurora-card-slow"} rounded-2xl p-5`}>
            <div className="flex items-center gap-2 mb-4">
              <TrendingUp className={`w-4 h-4 ${"text-white/30"}`} />
              <h3 className={`text-sm font-bold font-['Syne'] ${"text-white"}`}>Portfolio Intelligence</h3>
            </div>
            <div className="grid sm:grid-cols-3 gap-5 text-xs">
              <div>
                <div className={`mb-2 uppercase tracking-wide ${"text-white/30"}`}>Risk Distribution</div>
                {(["critical", "high", "medium", "low"] as const).map((level) => {
                  ...
                  return (
                    <div key={level} className="flex items-center gap-2 mb-1.5">
                      <div className={`w-20 h-1.5 rounded-full overflow-hidden ${"bg-white/[0.05]"}`}>
                      ...
                      <span className={`capitalize ${"text-white/40"}`}>{count} {level}</span>
                    </div>
                  );
                })}
              </div>
              <div>
                <div className={`mb-2 uppercase tracking-wide ${"text-white/30"}`}>Frameworks</div>
              ...
              <div>
                <div className={`mb-2 uppercase tracking-wide ${"text-white/30"}`}>Recommended Action</div>
