    ...
    return (
      <div className={`${t.page} flex items-center justify-center`}>
        <div className="text-center space-y-4">
          ...
          <Link href="/dashboard" className="inline-block text-sm text-violet-400 hover:underline mt-2">← Back to Dashboard</Link>
        </div>
      </div>
    );
  }
        ...
        {stats && (
          <>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                { label: "Total Users", value: stats.totalUsers, sub: `+${stats.usersThisMonth} this month`, icon: <Users className="w-4 h-4" />, color: "text-violet-400" },
                { label: "Total Scans", value: stats.totalScans, sub: `${stats.scansThisMonth} this month`, icon: <ScanLine className="w-4 h-4" />, color: "text-blue-400" },
                { label: "Avg Score", value: `${stats.avgScore}/100`, sub: "across all scans", icon: <Star className="w-4 h-4" />, color: "text-amber-400" },
                { label: "Completed", value: stats.statusBreakdown["completed"] ?? 0, sub: `of ${stats.totalScans} scans`, icon: <TrendingUp className="w-4 h-4" />, color: "text-green-400" },
              ].map((kpi, i) => (
                ...
                {stats.monthlyScans.map((m, i) => {
                  ...
                  return (
                    <div key={i} className="flex-1 flex flex-col items-center gap-2">
                      <span className={`text-xs font-semibold ${m.count > 0 ? "text-blue-400" : t.sub}`}>{m.count}</span>
