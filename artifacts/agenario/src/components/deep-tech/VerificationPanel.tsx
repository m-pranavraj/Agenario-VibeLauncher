...
export function VerificationPanel({ scan: rawScan, activeSection }: Props) {
  ...
  const scan = useMemo(() => {
    ...
    return {
      ...
      archScan: rawScan.archScan ?? null,
      symCost: rawScan.symCost ?? null,
      bigOProfiler: rawScan.bigOProfiler ?? null,
      timeAwareDeps: rawScan.timeAwareDeps ?? null,
      regGraph: rawScan.regGraph ?? null,
      flowValue: rawScan.flowValue ?? null,
      productReality: rawScan.productReality ?? null,
      realityCheck: rawScan.realityCheck ?? null,
       ...
       engines: [
         {
         ...
         },
          {
            ...
            icon: Wind,
            color: "teal",
            score: scan.entropyLeaks?.totalLeaks != null ? Math.max(0, 100 - (scan.entropyLeaks.totalLeaks as number) * 5) : 0,
            threshold: 60,
            ...
            why: "Secrets have measurably higher Shannon entropy than normal text. API keys, private keys show H(X) > 4.5 bits/char.",
            expected: "Zero high-entropy anomalies in non-crypto paths",
            actual: scan.entropyLeaks ? `${scan.entropyLeaks.totalLeaks ?? 0} leaks, avg H=${(scan.entropyLeaks.avgEntropy ?? 0).toFixed(2)}` : "Engine not run",
            proofRef: "advanced-math-engine.ts:238 H(X) = -Σ p(x)log₂p(x)",
            evidenceTier: 3 as const,
            details: scan.entropyLeaks ? [
              { label: "Entropy Leaks", value: String(scan.entropyLeaks.totalLeaks ?? 0) },
              { label: "Avg H(X)", value: (scan.entropyLeaks.avgEntropy ?? 0).toFixed(3) },
            ] : [],
            dataKey: "entropyLeaks",
            actionItems: !scan.entropyLeaks ? ["Rescan to enable entropy analysis"] : null,
          },
          {
            title: "AST Merkle Attestation",
            shortName: "MERK",
            icon: Key,
            color: "emerald",
            score: scan.astMerkle?.astLeafCount != null ? (scan.astMerkle.astLeafCount > 0 ? 95 : 0) : 0,
            threshold: 70,
            whyNeeded: "Cryptographic proof that source code existed at scan time.",
            whatItChecks: "SHA-256 Merkle tree over normalized AST lines.",
            proofPath: "ast-merkle-hasher.ts",
            why: "A Merkle tree over source lines produces a deterministic fingerprint. Change the code, the hash changes. Provides tamper-evident attestation.",
            expected: "Merkle root computed; leaf count > 0",
            actual: scan.astMerkle ? `Root: ${(scan.astMerkle.merkleRoot ?? "").slice(0, 16)}... ${scan.astMerkle.astLeafCount ?? 0} lines` : "Engine not run",
            proofRef: "ast-merkle-hasher.ts:74",
            evidenceTier: 3 as const,
            details: scan.astMerkle ? [
              { label: "Merkle Root", value: (scan.astMerkle.merkleRoot ?? "N/A").slice(0, 24) + "..." },
              { label: "Hashed Lines", value: String(scan.astMerkle.astLeafCount ?? 0) },
              { label: "Complexity", value: String(scan.astMerkle.cyclomaticComplexity ?? 0) },
            ] : [],
            dataKey: "astMerkle",
            actionItems: !scan.astMerkle || !scan.astMerkle.merkleRoot ? ["Rescan to compute Merkle attestation"] : null,
          },
      ],
    },
    {
